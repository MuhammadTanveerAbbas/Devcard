...
const PROFILE = {
  ...
  about:
    "I build your SaaS MVP in 14 days  auth, payments, and everything in between  so you can start getting users, not next quarter.",
...
export function ProfileCard() {
  ...
  useEffect(() => {
    const step = (ts: number) => {
      if (lastRef.current)
        setAngle((a) => (a + (ts - lastRef.current) * 0.12) % 360);
      lastRef.current = ts;
              ...
              <div className="px-4 sm:px-5 pb-5 pt-3 space-y-3.5">
                ...
                <div className="flex flex-wrap justify-center gap-1">
                  {PROFILE.skills.map((s) => (
                    <span
                      key={s}
                      className="px-2 py-0.5 text-[10px] sm:text-xs font-medium rounded-md bg-black/5 dark:bg-white/10 text-black/70 dark:text-white/70 border border-black/8 dark:border-white/10"
                    >
                      {s}
                    </span>
                  ))}
